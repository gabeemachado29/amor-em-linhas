<?php
    session_start(); // CORREÇÃO: Adicionado session_start() que faltava
    ...
    if($_POST){
        ...
        /* GERA PIX PRIMEIRO PARA SALVAR NO BANCO */
        ...
        foreach($carrinho as $id => $qtd){
            $produto = $db->query("SELECT * FROM produtos WHERE id = $id")->fetch(PDO::FETCH_ASSOC);
            $stmt = $db->prepare("INSERT INTO itens_pedido (pedido_id, produto_id, quantidade, preco_unitario) VALUES (?, ?, ?, ?)");
